...
export type DecrementInstruction<
  ...
  TAccountIdngenelectevotingsolana extends string | AccountMeta<string> = string,
...
export type DecrementInput<TAccountIdngenelectevotingsolana extends string = string> = {
  idngenelectevotingsolana: Address<TAccountIdngenelectevotingsolana>;
};

export function getDecrementInstruction<
  TAccountIdngenelectevotingsolana extends string,
  TProgramAddress extends Address = typeof IDNGENELECTEVOTINGSOLANA_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountIdngenelectevotingsolana>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountIdngenelectevotingsolana> {
  // Program address.
  const programAddress = config?.programAddress ?? IDNGENELECTEVOTINGSOLANA_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    idngenelectevotingsolana: { value: input.idngenelectevotingsolana ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.idngenelectevotingsolana)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountIdngenelectevotingsolana>;
...
> = {
  ...
  accounts: {
    idngenelectevotingsolana: TAccountMetas[0];
  };
